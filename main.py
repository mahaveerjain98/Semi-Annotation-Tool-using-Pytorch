"""
SemiAuto Annotatot using Pytorch
"""
...
"""
Add packages here of torch which needed to use
"""
import torch
import torchvision
# import fc_model
...
class MainGUI:
    def __init__(self, master):
        ...
        self.torch_ = 1 if it is needed or else leave it
        """
    ...
    """
    No need of tf sessions in pytorch
    """
            ...
            if(self.modelIntVars[listidxmodel].get()):
                ...
                if('pytorch' in list_model_name):
                    ...
                    if 'yolo' in self.model_path:
                        ...
                        print(str(os.path.basename(self.model_path)))
                        try:
                            self.model = torch.hub.load('pytorch/vision', str(os.path.basename(self.model_path)), pretrained=True)
                        except Exception as e:
                            raise e                    
                    

                # elif('keras' in list_model_name):
                #     self.keras_ = 1
                #     self.tensorflow_ = 0
    ...
    # def load_checkpoint_pytorch(self,filepath):
        # for yolov5 only
        # device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
        # model = torch.hub.load('ultralytics/yolov5', 'custom', path_or_model=filepath)
        # checkpoint = torch.load(filepath,map_location=device)
        # chk =checkpoint.float().state_dict()
        # #intesection
        # chk={k: v for k, v in chk.items() if k in model.state_dict() and v.shape == model.state_dict()[k].shape}
        # ## Model definition
        # #have to check does every model inside it has input size, output size and hidden layer
        # # model = fc_model.Network(checkpoint['input_size'],
        # #                         checkpoint['output_size'],
        # #                         checkpoint['hidden_layers'])
        # model.load_state_dict(chk,strict=False)
        # return model
    # def preprocess_image_pytorch(self,img):
    #     device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    ...
    def automate(self):
        ...
        # print(self.filenameBuffer)
        # print(self.imageDirPathBuffer + '/' + self.imageList[self.cur])
        # print(type(self.imageDirPathBuffer + '/' + self.imageList[self.cur]))
        # image = self.preprocess_image_pytorch(opencvImage)
        results = self.model("img.jpg")
        os.remove("img.jpg")
        # print(results.pred)
        boxes=[]
        scores=[]
        labels=[]
        for i in range(len(results.pred[0].numpy())):
            boxes.append([int(results.pred[0].numpy()[i][0]),int(results.pred[0].numpy()[i][1]),int(results.pred[0].numpy()[i][2]),int(results.pred[0].numpy()[i][3])])
            scores.append(results.pred[0].numpy()[i][4])
            labels.append(int(results.pred[0].numpy()[i][5]))

        # x1=pred.pred[0][0].numpy()[0]
        # y1=pred.pred[0][0].numpy()[1]
        # x2=pred.pred[0][0].numpy()[2]
        # y2=pred.pred[0][0].numpy()[3]
        # b=[int(x1),int(y1),int(x2),int(y2)]
        predd={}
        for i, lab in enumerate(results.names):
            predd[i]=lab
        config_labels=predd
        ...
        for idx, (box, label, score) in enumerate(zip(boxes, labels, scores)):
            ...
            self.objectListBox.insert(END, '(%d, %d) -> (%d, %d)' % (b[0], b[1], b[2], b[3]) + ': ' +str(config_labels[label])+' '+str(int(score*100))+'%'
                                        +' '+ m_name)
            self.objectListBox.itemconfig(len(self.bboxIdList) - 1,
